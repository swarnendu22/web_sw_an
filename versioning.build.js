...
var indexPath = (__dirname + buildPath + '/' + 'index.html').replace('\\', '/');
console.log('Angular build path:', (__dirname + buildPath).replace('\\', '/'));
console.log('Change by buildVersion:',buildVersion);
fs.readdir((__dirname + buildPath).replace('\\', '/'), (err, files) => {
  files.forEach(file => {
    if (file.match(/^(es2015-polyfills|vender|main|polyfills|runtime|scripts|styles)+(.)+([a-z0–9.\-])*(js|map|css)$/g)) { // regex is identified by build files generated
