...
import { UploadBulkPrice } from 'src/app/actions/seller-catalog-action';
...
@Component({
  ...
  styleUrls: ['./bulk-price-upload.component.css']
})
export class BulkPriceUploadComponent implements OnInit {
  ...
  upload(event) {
    ...
    this.store.dispatch(new UploadBulkPrice(
      {
    ...
    this.apiMessageService.currentApiStatus.subscribe((response) => {
      ...
      if (response.type == 'uploadBulkPrice') {
