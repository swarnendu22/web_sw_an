...
@Component({
  selector: 'app-manage-merchents',
  templateUrl: './manage-merchents.component.html',
  styleUrls: ['./manage-merchents.component.css'],
  providers: [NgbModalConfig, NgbModal],
})
export class ManageMerchentsComponent implements OnInit {
  ...
  constructor(
    ...
    this.columnDefs = [
      ...
      //   headerName: 'Profile %',
      //   cellRenderer: (data) => {
      //     if (data.data.profileCompletionPercent)
      //       return parseInt(data.data.profileCompletionPercent)
      //   }
      // },
      {
  ...
  ngOnInit() {
    this.payLoadForSearch.status = 'APPROVED';
    ...
    if (params) {
      ...
      dialog.afterClosed().subscribe(result => {
        if (result.processType === 'process') {
          ...
          this.store.dispatch(new StoreApproveReject({ pageNo: this.pageNo, requestBody: this.payLoadForStoreApproceReject, searchPayLoad: this.payLoadForSearch }));
          this.apiMsgService.currentApiStatus.subscribe((response) => {
            ...
            if (res && response.type == ActionTypes.storeApproveReject) {
              ...
              if (this.payLoadForSearch && this.payLoadForSearch.status) {
                ...
                this.addCheckBoxInColumnDefination();
              } else {
