...
import { OrderListingByStatus } from 'src/app/actions/delivery-boy-management.action';
import { Store, select } from '@ngrx/store';


@Component({
  selector: 'app-delivery-boy-order-table',
  templateUrl: './delivery-boy-order-table.component.html',
  styleUrls: ['./delivery-boy-order-table.component.css'],
})
export class DeliveryBoyOrderTableComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.store.dispatch(new OrderListingByStatus({ status: 'active_orders' }));
    this.store
      .pipe(select('deliveryBoyManagement'))
      .subscribe(res => {
        if (res.orderListingByStatus) {
          console.log('ORDER LISTING', res.orderListingByStatus)

          this.totalOrderListing = res.orderListingByStatus['seller_orders'];
          // event.api.setRowData(this.rowData);


        }
      });
    // console.log('SORT NG ONIT', this.sort)
  ...
  ngAfterViewInit() {
    ...
    const rows$ = this.totalOrderListing

    // this.totalRows$ = rows$.pipe(map(rows => rows.length));
    // this.displayedRows$ = rows$;
    // this.totalRows$ = this.totalOrderListing.length
    // this.displayedRows$ = this.totalOrderListing;
  }
