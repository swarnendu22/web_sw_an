...
@Component({
  selector: 'app-platform-coupon',
  templateUrl: './platform-coupon.component.html',
  styleUrls: ['./platform-coupon.component.css']
})
export class PlatformCouponComponent implements OnInit {
  couponCodeVal = null;
  pageNo = 1;
  perPage = 20;
  totalRecords = 0;
  ...
  searchByCode() {
    this.store.dispatch(new GetMasterCoupons({
      ...
      coupon_category: 'PLATFORM_OFFER',
      perPage: this.perPage,
      pageNo: this.pageNo
    }));
  }
  addCoupon() {
    sessionStorage.setItem('coupon_category', 'PLATFORM_OFFER');
