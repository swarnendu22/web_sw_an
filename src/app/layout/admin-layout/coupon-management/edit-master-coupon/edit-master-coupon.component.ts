...
@Component({
  selector: 'app-edit-master-coupon',
  templateUrl: './edit-master-coupon.component.html',
  styleUrls: ['./edit-master-coupon.component.css'],
  providers: [
    {
      provide: DateAdapter,
...
export class EditMasterCouponComponent implements OnInit {
  ...
  constructor(
    ...
    this.store.dispatch(new GetCouponById(this.route.snapshot.params.id));
  }
  ...
  ngOnInit(): void {
    ...
    this.couponForm = this.fb.group({
      ...
      coupon_category: ['SELLER_PROMO', Validators.required],
    ...
    .subscribe(res => {
      if (res.couponDetailsById) {
        this.couponDetailsById = res.couponDetailsById;
        this.setformDetails();
      }
    });
    this.store
  ...
  setformDetails() {
    this.couponForm.get('code').setValue(this.couponDetailsById.code);
    this.couponForm.get('coupon_category').setValue(this.couponDetailsById.coupon_category);
    this.couponForm.get('description').setValue(this.couponDetailsById.description);


    if(this.couponDetailsById.start_date != null) {
      this.couponForm.get('start_date').setValue(new Date(this.couponDetailsById.start_date));
      this.minDate = new Date(this.couponDetailsById.start_date);
    }
    if(this.couponDetailsById.end_date != null) {
      this.couponForm.get('end_date').setValue(new Date(this.couponDetailsById.end_date));
    }

    if(new Date(this.couponDetailsById.end_date).getTime() >= new Date('2025/12/31').getTime()) {
      this.neverExpired = true;
    } else {
    ...
    this.couponForm.get('coupon_type').setValue(this.couponDetailsById.coupon_type);
    this.couponForm.get('platforms').setValue(this.couponDetailsById.platforms);
    if(this.couponDetailsById.list_visible) {
      this.couponForm.get('list_visible').setValue('Yes');
    } else {
      this.couponForm.get('list_visible').setValue('No');
    }
    
    this.couponForm.get('active').setValue(this.couponDetailsById.active);
    this.couponForm.get('cod_available').setValue( this.couponDetailsById.cod_available);
    
    if(this.couponDetailsById.coupon_condition) {
      this.couponForm.get('business_catagories').setValue(this.couponDetailsById.coupon_condition.business_catagories);
      this.couponForm.get('business_sector').setValue(this.couponDetailsById.coupon_condition.business_sector);
      this.businessSectorChanged();
      this.couponForm.get('store_subscription').setValue(this.couponDetailsById.coupon_condition.store_subscription);
      this.couponForm.get('store_visibility').setValue(this.couponDetailsById.coupon_condition.store_visibility);
    }

    if(this.couponDetailsById.conditions) {
      
      if(this.couponDetailsById.conditions.rules) {
        
        if(this.couponDetailsById.conditions.rules[0]) {
          if(this.couponDetailsById.conditions.rules[0].cart_attributes) {
            if(this.couponDetailsById.conditions.rules[0].cart_attributes[0]) {
              if(this.couponDetailsById.conditions.rules[0].cart_attributes[0].value==0) {
                this.couponForm.get('ordersType').setValue('ALL');
                this.couponForm.get('cart_attributes_value').setValue('');
              } else {
                this.couponForm.get('ordersType').setValue('SUBTOTAL');
                this.couponForm.get('cart_attributes_value').setValue(this.couponDetailsById.conditions.rules[0].cart_attributes[0].value);
              }
            }
          }
        }
    ...
    if(this.couponDetailsById.actions) {
      this.couponForm.get('discount_type').setValue(this.couponDetailsById.actions.discount_type);
      this.couponForm.get('discount_amount').setValue(this.couponDetailsById.actions.discount_amount);
      this.couponForm.get('discount_max_upto').setValue(this.couponDetailsById.actions.discount_max_upto);
    }
    this.coupon_discount_type();
    if(this.couponDetailsById.restriction_per_user==1) {
      this.couponForm.get('number_of_uses').setValue('one');
    }
    if(this.couponDetailsById.restriction_per_user > 1) {
      this.couponForm.get('number_of_uses').setValue('max_upto');
      this.couponForm.get('restriction_per_user').setValue(this.couponDetailsById.restriction_per_user);
    } else {
      this.couponForm.get('restriction_per_user').setValue('');
    }
    this.couponForm.get('coupon_effect_seller_nsp').setValue(this.couponDetailsById.coupon_effect_seller_nsp);
    
    this.couponForm.get('terms_and_conditions').setValue(this.couponDetailsById.terms_and_conditions);
  
    this.coupon_asset_url = this.couponDetailsById.coupon_asset;
    this.coupon_response_asset_url = this.couponDetailsById.coupon_response_asset;
    this.coupon_response_asset_type = this.couponDetailsById.coupon_response_asset_type;
    ...
    if(this.couponDetailsById.coupon_category=='SPECIAL_PROMO') {
    ...
    if (this.couponForm.valid) {
      if ( new Date(this.couponForm.get('end_date').value).getTime() >  new Date(this.couponForm.get('start_date').value).getTime()) {
        ...
        if(this.couponForm.get('terms_and_conditions').value != '') 
        {
          ...
          {
            if(this.couponDetailsById.coupon_condition != null) {
              coupon_condition_attributes = {
                "id": this.couponDetailsById.coupon_condition.id,
          ...
          if(isCouponPayment && isConditionsValue)
          {
            ...
            this.store.dispatch(new EditMasterCouponCode(payload));
            this.subscriptionApi = this.apiMessageService.currentApiStatus.subscribe((data:any) => {
              if (data.status === true && data.type ==  ActionTypes.editMasterCouponCode) {
                setTimeout(()=>{
                  if(this.couponForm.get('coupon_category').value=='SELLER_PROMO') {
  ...
  public paymentMethodList() {
    const dialog = this.dialog.open(PaymentMethodListComponent, {
      width: '600px',
      height: '400px',
      disableClose: true,
      panelClass: 'ndh-order-view',
      data: this.couponDetailsById.coupon_payment_conditions
    })
  ...
  businessSectorChanged() {
    ...
    {
      if(this.businessCategoryList != null) {
